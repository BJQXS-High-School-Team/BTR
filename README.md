...
> **BTR：Battery Thermal Runaway Early-warning System**  
> 面向电动车锂电池充电、存放与使用过程中的热失控风险，本项目构建了一套融合 **多源传感器感知、ESP32 边缘判断、网页端可视化、MQTT 通信、桌面端 AI 在线推理与状态回传纠偏** 的智能早期预警系统。

---

## 🏆 作品定位

本作品面向人工智能与开源硬件结合类竞赛，核心目标不是简单地“温度过高报警”，而是通过多源传感器连续采集锂电池周边状态数据，并利用时序预测和机器学习模型识别热失控风险趋势，实现从**事后报警**到**事前预警**的转变。

项目重点体现：

- **真实问题导向**：面向电动车锂电池热失控导致的火灾、爆燃与公共安全风险。
- **多源感知**：融合温度、湿度、气体、VOC、烟雾/光学信号等多维数据。
- **AI 在线推理**：支持 RF、LSTM、CNN-LSTM、Transformer 等模型训练与在线预测。
- **端云协同闭环**：ESP32 本地快速判断，电脑端/云端 AI 推理后可回传状态纠偏。
- **可展示与可部署**：具备网页端、桌面端、OLED、蜂鸣器、RGB 灯等多种展示与报警方式。
...
系统不仅采集电池温度，还同步采集环境温湿度、MQ 系列气体、VOC 指标、烟雾/光学信号等数据，避免单一温度阈值带来的误报与漏报。

### 2. 时序预测与风险趋势识别

热失控是一个持续发展的过程，系统维护历史数据窗口，并通过 RF、LSTM、CNN-LSTM、Transformer 等模型分析连续数据变化趋势，识别风险是否正在形成。

### 3. ESP32 边缘判断 + 电脑端 AI 推理

ESP32 端可独立完成传感器采集、本地三级状态判断和报警；电脑端通过 MQTT 接收实时数据，调用 AI 模型进行在线推理，并在必要时将修正状态回传给 ESP32。

### 4. 双界面展示：网页端 + 桌面端

- **网页端**：用于现场监控、传感器数值查看、趋势曲线、阈值配置、AP 配网与 SPIFFS 管理。
- **桌面端**：用于 MQTT 数据接收、实时曲线显示、随机森林在线推理、风险概率显示与状态回传纠偏。

### 5. 基于真实测试数据训练模型

项目训练数据包括项目组自主测试数据，并结合合作实验室授权的真实测试数据，用于提升模型对真实工况下异常升温与热失控风险的识别能力。
...
Python 端接收并构建时序窗口
    ↓
RF / LSTM / CNN-LSTM / Transformer 在线推理
    ↓
输出热失控风险概率与风险等级
    ↓
必要时通过 MQTT 回传 normal / warning / danger 状态
    ↓
ESP32 执行 OLED 显示、RGB 灯、蜂鸣器报警与状态锁存
```
...
网页端运行在 ESP32 内置 WebServer 中，主要用于现场调试与部署：

- 实时显示温度、湿度、气体、VOC、烟雾等传感器数据。
- 显示 `NORMAL` / `WARNING` / `DANGER` 当前状态。
- 绘制最近一段时间的传感器变化曲线。
- 支持手动修改阈值。
- 支持基于近期数据自动生成阈值。
...
桌面端位于 `computer/rf_status_app.py`，主要用于 AI 推理与状态纠偏：

- 通过 MQTT 订阅 ESP32 上传的 `sensor` 数据。
- 实时显示内部温度、外部温度、湿度、MQ、VOC、烟雾等数据。
- 绘制实时变化曲线。
- 使用随机森林模型输出热失控风险概率。
- 当 AI 判断与 ESP32 本地状态不一致时，可向 `statue` 主题回传纠偏状态。
- 支持手动发送 `normal` / `danger`，便于演示系统联动与报警逻辑。

---

## 📌 一句话介绍

BTR 是一套基于 **ESP32 多源传感器采集 + MQTT 实时传输 + Python AI 模型推理 + 网页/桌面双端可视化 + 状态回传纠偏** 的电动车锂电池热失控早期预警系统。
...
项目数据主要用于训练和验证热失控风险识别模型，包括正常状态、环境扰动、异常升温、气体/VOC/烟雾变化等多类时序数据。

数据来源包括：

- 项目组自主测试采集数据。
- 合作实验室授权的真实测试数据。
- 仓库 `computer/data/` 与 `data_backup_*` 中保存的训练与备份数据。

已实现或预留的模型路线：

| 模型 | 文件 | 作用 |
|------|------|------|
| Random Forest | `train_model_random_forest.py` / `main_RF.py` | 可解释性强，适合桌面端在线推理与风险概率输出 |
| LSTM | `train_modelLSTM.py` / `main_LSTM.py` | 面向连续温度时序变化趋势预测 |
| 60s LSTM | `train_model_lstm_60s.py` | 面向更长时间窗口的趋势识别 |
| CNN-LSTM | `train_model_cnnlstm.py` / `main_cnn_lstm.py` | 结合局部特征提取与时序建模 |
| Transformer | `train_model_transformer.py` / `main_transformer.py` | 捕捉较长序列中的关联特征 |

> 对比赛展示而言，建议重点说明：随机森林用于桌面端实时推理与可解释展示，LSTM/CNN-LSTM/Transformer 用于体现时序预测与模型对比能力。
...
1. **正常状态展示**：打开 ESP32 网页端和电脑桌面端，显示 `NORMAL`。
2. **多源数据采集**：展示温度、湿度、MQ、VOC、烟雾等实时数值和曲线。
3. **模拟异常变化**：使用安全方式模拟升温或气体/VOC 波动。
4. **AI 在线推理**：桌面端显示 RF 风险概率变化。
5. **状态回传纠偏**：展示电脑端向 ESP32 下发 `danger` 或 `normal`。
6. **声光报警联动**：OLED、RGB 灯、蜂鸣器同步响应。
...
BTR 的核心价值在于：它将锂电池热失控预警从单一阈值报警升级为多源数据融合与 AI 趋势判断。系统既能在 ESP32 端独立完成边缘报警，也能通过电脑端模型进行在线推理和状态纠偏，形成“**监测—分析—反馈—预警**”闭环。

面向未来，本项目可进一步应用于电动车电池仓、充电柜、校园/社区电池集中存放点、储能柜等场景，为锂电池安全管理提供低成本、可部署、可扩展的智能预警方案。
