...
用 LSTM 模型预测热失控概率，并与 ESP32 自检状态进行双向纠正。
"""
...
# ====== LSTM 模型目录（根据训练时设置的目录修改）======
MODEL_DIR = "models_lstm_60s"   # 如果训练未来60秒的模型，目录可能是 models_lstm_60s
...
def load_lstm_model(model_dir):
    ...
    model = keras.models.load_model(os.path.join(model_dir, 'lstm_60s_model.h5'))
...
# 尝试加载模型
try:
    MODEL_LSTM, SCALER_LSTM, CONFIG = load_lstm_model(MODEL_DIR)
    ...
    print(f"LSTM 模型加载成功，序列长度: {SEQ_LEN}，最佳阈值: {BEST_THRESHOLD}")
...
def preprocess_and_predict(seq):
    ...
    try:
        ...
        # 标准化
        # SCALER_LSTM 是在训练时对 (n_samples * seq_len, n_features) 拟合的，
        # 因此需要对当前序列进行相同的转换：将 X_multi 展平为 (seq_len * n_features,)
        # 但更简单：SCALER_LSTM 期望输入形状 (n_samples, n_features)，其中 n_samples 为展平后的点数。
        # 由于 SCALER_LSTM 是按特征通道分别标准化的，我们需要对每个特征通道分别应用 transform。
        # 正确做法：将 X_multi reshape 为 (seq_len, n_features)，然后对每个特征列使用 scaler 的 mean_ 和 scale_。
        # 但更方便：直接使用 scaler.transform 要求输入形状为 (n_samples, n_features)，其中 n_samples 是样本数，
        # 这里每个时间步视为一个样本，因此将 X_multi reshape 为 (seq_len, n_features)，调用 transform 即可。
        X_reshaped = X_multi.reshape(-1, X_multi.shape[1])   # (seq_len, n_features)
        X_scaled = SCALER_LSTM.transform(X_reshaped)         # (seq_len, n_features)
        X_scaled = X_scaled.reshape(1, SEQ_LEN, -1)          # (1, seq_len, n_features)

        # 预测
        prob = MODEL_LSTM.predict(X_scaled, verbose=0)[0, 0]
        return float(prob)
    except Exception as e:
        print(f"预处理或预测出错: {e}")
        return None
