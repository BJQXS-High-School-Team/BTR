"""
电池热失控实时预测 - CNN + LSTM 版本
从 MQTT 接收传感器数据（内部温度、外部温度、ESP32状态），
对两个温度序列分别进行预处理（添加差分特征、标准化），
用 CNN+LSTM 模型预测热失控概率，并与 ESP32 自检状态进行双向纠正。
"""
...
# ====== CNN+LSTM 模型目录（根据训练时设置的目录修改）======
MODEL_DIR = "models_cnn_lstm"   # 训练 CNN+LSTM 时使用的目录
...
# ====== 加载 CNN+LSTM 模型、标准化器和配置 ======
def load_cnn_lstm_model(model_dir):
    ...
    model = keras.models.load_model(os.path.join(model_dir, 'cnn_lstm_model.h5'))
...
# 尝试加载模型
try:
    MODEL_CNN_LSTM, SCALER_CNN_LSTM, CONFIG = load_cnn_lstm_model(MODEL_DIR)
    ...
    print(f"CNN+LSTM 模型加载成功，序列长度: {SEQ_LEN}，最佳阈值: {BEST_THRESHOLD}")
    print(f"使用差分特征: {USE_DIFF_FEATURES}, 平滑: {USE_SMOOTHING}")
except Exception as e:
    print(f"加载模型失败: {e}")
    MODEL_CNN_LSTM = None
    SCALER_CNN_LSTM = None
    SEQ_LEN = 60
    BEST_THRESHOLD = 0.5
...
def preprocess_and_predict(seq):
    ...
    try:
        ...
        # 标准化：SCALER_CNN_LSTM 是按特征通道拟合的，需要对每个时间步的特征进行转换
        X_reshaped = X_multi.reshape(-1, X_multi.shape[1])   # (seq_len, n_features)
        X_scaled = SCALER_CNN_LSTM.transform(X_reshaped)     # (seq_len, n_features)
        X_scaled = X_scaled.reshape(1, SEQ_LEN, -1)          # (1, seq_len, n_features)

        # 预测
        prob = MODEL_CNN_LSTM.predict(X_scaled, verbose=0)[0, 0]
        return float(prob)
    except Exception as e:
        print(f"预处理或预测出错: {e}")
        return None
...
def process_and_predict(esp32_status):
    ...
    if MODEL_CNN_LSTM is None or SCALER_CNN_LSTM is None:
        return None
